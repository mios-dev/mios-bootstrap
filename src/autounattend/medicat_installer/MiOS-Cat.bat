...
.DriveType -eq 'Fixed' -and @echo off
title MiOS-Cat Dedicated USB Installer
cd /d %~dp0
set "maindir=%CD%"
:: Resolve dynamic configuration from mios.toml (SSOT)
...
.SizeRemaining -gt 25GB } | Sort-Object SizeRemaining -Descending | Select-Object -First 1); if ($v) { $v.DriveLetter + ':\MiOS\medicat_stage' } else { $env:TEMP + '\medicat_stage' }"") do set "stage_dir=%%i"
