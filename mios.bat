...
@echo off
:: mios.bat -- the MiOS workhorse. This file IS the installer.
:: cmd portion: invoke powershell on this same file; PowerShell
:: parses the surrounding block-comment as a no-op and runs the
:: body below. No separate fetch -- the install logic lives HERE.
%SystemRoot%\System32\WindowsPowerShell\v1.0\powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "& { Set-Location -LiteralPath '%CD%'; & ([scriptblock]::Create([System.IO.File]::ReadAllText('%~f0'))) }" %* 2>/dev/null
exit /b %ERRORLEVEL%
#>
<#
.SYNOPSIS
