...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$cd='%drivepath%'; $rd='%repodrive%'; $dd='%datadrive%'; Add-Type -AssemblyName System.Drawing -EA SilentlyContinue; function New-Ico($p, $lbl) { try { $bmp=New-Object System.Drawing.Bitmap 256,256; $g=[System.Drawing.Graphics]::FromImage($bmp); $g.SmoothingMode='AntiAlias'; $g.Clear([System.Drawing.Color]::FromArgb(40,34,98)); $b=New-Object System.Drawing.SolidBrush([System.Drawing.Color]::FromArgb(243,92,21)); $g.FillEllipse($b,24,24,208,208); $pen=New-Object System.Drawing.Pen([System.Drawing.Color]::FromArgb(231,223,211),6); $g.DrawEllipse($pen,24,24,208,208); $fM=New-Object System.Drawing.Font('Arial',96,[System.Drawing.FontStyle]::Bold); $sf=New-Object System.Drawing.StringFormat; $sf.Alignment=[System.Drawing.StringAlignment]::Center; $sf.LineAlignment=[System.Drawing.StringAlignment]::Center; $g.DrawString('M',$fM,[System.Drawing.Brushes]::White,(New-Object System.Drawing.RectangleF 0,0,256,230),$sf); if ($lbl){ $fL=New-Object System.Drawing.Font('Consolas',16,[System.Drawing.FontStyle]::Bold); $g.DrawString($lbl.ToUpper(),$fL,[System.Drawing.Brushes]::LightGray,(New-Object System.Drawing.RectangleF 0,195,256,45),$sf) }; $dir=[System.IO.Path]::GetDirectoryName($p); if (-not (Test-Path $dir)){ New-Item -ItemType Directory -Force -Path $dir|Out-Null }; $fs=New-Object System.IO.FileStream($p,[System.IO.FileMode]::Create); [System.Drawing.Icon]::FromHandle($bmp.GetHicon()).Save($fs); $fs.Close() } catch{} }; function Stage-Ico($let,$lbl){ if (-not $let -or $let -eq '_'){ return }; $r=\"$($let.Trim(':')):\"; if (-not (Test-Path $r)){ return }; $a=Join-Path $r 'autorun'; $ico=Join-Path $a 'mios.ico'; New-Ico $ico $lbl; $inf=Join-Path $r 'autorun.inf'; [System.IO.File]::WriteAllText($inf, \"[autorun]`nicon=autorun\mios.ico`nlabel=$lbl\", [System.Text.Encoding]::ASCII); try { Set-ItemProperty -Path \"HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\DriveIcons\$let\DefaultIcon\" -Name '(default)' -Value $ico -Force -EA SilentlyContinue } catch{} }; Stage-Ico $cd 'MiOS-Cat'; Stage-Ico $rd 'MiOS-Repo'; Stage-Ico $dd 'MiOS-Data'" >nul 2>&1
