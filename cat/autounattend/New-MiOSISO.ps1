...
function Invoke-MiOSLiveMonitorAuto {
    try {
        $monScript = Join-Path $PSScriptRoot 'Monitor-MiOSFlash.ps1'
        if (Test-Path $monScript) {
            $existing = Get-Process powershell -ErrorAction SilentlyContinue | Where-Object { $_.CommandLine -like '*Monitor-MiOSFlash*' }
            if (-not $existing) {
